---
title: "Donnees Quotidiennes Des Cas de COVID 19 Au Senegal"
author: epinotes
output: 
  flexdashboard::flex_dashboard:
    orientation: rows
    vertical_layout: scroll
    theme: bootstrap  
runtime: shiny
---

<!-- https://epinotes.shinyapps.io/upload_file_for_OD_coding/ -->

```{r global, include=FALSE}
# load data in 'global' chunk so it can be shared by all users of the dashboard
library(DT)
library(tidyverse)
library(flexdashboard)
library(shiny)
library(tsibble)
library(tidycovid19)
library(ggthemes)
library(plotly)

# library(shinydashboard)
# dashboardBody( tags$head( tags$meta(name = "viewport", contentwidth=1600"),uiOutput("body")))

source("R/sencovid19.R")

decedes <- max(sencovid_cumule$total_decedes) 
gueris <- last(sencovid_cumule$total_gueris) 
total <- last(sencovid_cumule$total_confirmes)

testes <- last(sencovid_cumule$nombre_testes)




```



Sommaire
=====================================

</br>
Les donnees proviennent principalement des communiques officiels du ministere de la sante du gouvernement du Senegal. 
  Les dernieres donnees sont en date du __`r last_date `__.   
  En ce meme jour __`r testes `__ personnes ont ete testees resultant en __`r today_pos` (`r percent(today_pos/testes)`)__ cas positifs.   
  Ouvrez les onglets ci-dessus pour les courbes epidemiologiques, la comparison avec d'autres pays, et les sources d'information y compris les codes `R`. 
    
  
</br>


 
    
Row
-------------------------------------   


### Gueris  
    
```{r}
valueBox(value = gueris,
         icon = "fa-thumbs-up",
         color = "#35B779FF")
 

```

### Confirmes  
    
```{r}
flexdashboard::valueBox(value = total,
                        icon = "fa-area-chart",
                        color =  "#FDE725FF")
 

```


### Decedes  

```{r}
valueBox(value = decedes,
         icon = "fa-heartbeat",
         color = "#F1605DFF")

```





Courbe Epi - Incidence
=====================================


### Courbe Epi - Incidence 


```{r}


fluidPage( 
mainPanel(
  plotlyOutput("plot2", width = "100%")))

output$plot2 <- renderPlotly({
print(sen_epic2 %>% layout(width = 1400 ))
})

```

Courbe Epi - Cumulee
=====================================
  

### Epicurve - Cumulative


```{r}


fluidPage( 
mainPanel(
  plotlyOutput("plot3")))

output$plot3 <- renderPlotly({
print(cum_epi)
})

```

Sources
=====================================   

</br>

Row
-------------------------------------   

### Communiques Quotidiens  

[Communiques Quotidiens Du Ministere De La Sante Du Senegal](http://www.sante.gouv.sn/Pr%C3%A9sentation/coronavirus-informations-officielles-et-quotidiennes-du-msas){target="_blank"}

</br>

Voir aussi la page [GitHub](https://github.com/epinotes/sencovid19) pour les codes R.

Row
-------------------------------------   

### Tableau Des Cas Confirmes Par Jour

```{r}

fluidPage(DTOutput('tbl'))
  
    output$tbl = renderDT(
      covid19_daily %>% 
  DT::datatable(caption = "Nombre de Cas Confirmes par Date",
                
                rownames = FALSE,
                
                extensions = 'Buttons',
                
                filter = list(position = 'top', clear = FALSE),
                
                options = list(
                  
                  pageLength = 10, dom = 'Bfrtip',
                  
                  buttons = c('copy', 'csv'),
                  
                  columnDefs = list(list(className = 'dt-left', targets = 1))
                  
                ))
    )

```

